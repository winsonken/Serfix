...
const RegisterScreen = () => {
    ...
    return (
        <View className="flex flex-1 bg-main-background px-5" style={{ paddingTop: insets.top }}>
            ...
            <PopUpErrorRegister title="Registration Error" content={errorMessages} isOpenPopUp={isOpenPopUpError} setIsOpenPopUp={setIsOpenPopUpError} />
...
export function PopUpRegisterSuccess(props) {
  ...
  export function PopUpErrorRegister(props) {
    const { title, content, isOpenPopUp, setIsOpenPopUp } = props;
            ...
            <View className="bg-white w-4/5 h-fit min-h-20 flex justify-between rounded-md p-2">
              <View>
                  ...
                  <View className="py-3 flex justify-center items-center">
                      <Text className={`${(content == 'An unexpected error occurred. Please try again.') ? 'text-center' : 'text-left'}`}>{content}</Text>
                  </View>
              </View>
